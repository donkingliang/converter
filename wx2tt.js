					...
					if (eror) {
						...
						if (isFile) {
							...
							if (regml.test(filedir)) {
								let newPath = filedir.replace(regml, 'ttml');
							...
							//如果是wxs的文件转成sjs
							if (regwxs.test(filedir)) {
								let newPath = filedir.replace(regwxs, 'sjs');
								reNameFile(filedir, newPath)
								amendWxToTT(filedir)
							}
...
function amendText(path, callback) {
	fs.readFile(path, 'utf8', function(err, files) {
		...
		var result = files.replace(/wx:for/g, 'tt:for')
		.replace(/wx:if/g, 'tt:if')
		.replace(/wx:for-item/g,'tt:for-item')
		.replace(/wx:for-index/g, 'tt:for-index')
		.replace(/wx:key/g, 'tt:key')
		.replace(/\.wxml/g, '.ttml')
		.replace(/\.wxs/g, '.sjs')
		.replace(/<wxs/g, '<sjs')
		.replace(/<\/wxs>/g, '</sjs>')
